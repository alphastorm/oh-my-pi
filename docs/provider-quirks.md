...
- **Provider Protocol & Request Envelope**:
  ...
  - **System Instructions**: Antigravity tags system instructions with `role: "user"` and preserves caller text. No identity prompt is injected. Reports in [#11809](https://github.com/can1357/oh-my-pi/issues/11809) show prompt-dependent rejection; arbitrary prompt acceptance is not guaranteed.
  - **Request Envelope & Session State**: `buildRequest` adds `project` (projectId) and `userAgent` (`antigravity`); `buildAntigravityRequestEnvelope` supplies `requestId` (`agent/<agentId>/<ts>/<trajectoryId>/<step>`), `sessionId`, and `labels` (`last_step_index`, `model_enum`, `trajectory_id`, `used_claude`, `used_claude_conservative`, `last_execution_id`). The private downstream build omits `requestType` for chat, image generation, and search. State maintains monotonic `stepIndex`, persistent `agentId`, `trajectoryId`, and signed-decimal `sessionId` (`deriveAntigravitySessionId`).
...
- **System Instruction & Request Envelope**: Antigravity tags `systemInstruction` with `role: "user"` and sends the caller's prompts unmodified. `buildAntigravityRequestEnvelope` supplies structured `requestId` (`agent/<id>/<ts>/<trajectoryId>/<step>`), `sessionId`, and `labels` (`model_enum`, `trajectory_id`, `last_step_index`, `last_execution_id`, `used_claude*`) using `getAntigravityModelWireProfile`; `buildRequest` adds `userAgent: "antigravity"`.
- **Private Downstream Workaround**: This build follows [PR #11742](https://github.com/can1357/oh-my-pi/pull/11742) by omitting `requestType` from all three Antigravity serializers. Prompts, tools, quota classification, and retry policy are unchanged. Upstream declined this bypass because of targeted-block escalation risk; account restrictions or renewed blocking remain possible. Provider-free serializer verification does not establish live Google acceptance.
