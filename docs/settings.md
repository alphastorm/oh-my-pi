...
| `providers.openai-codex.cyberAccess`         | enum    | `auto`            | Cyber access program sent as `access_programs.cyber` on first-party `openai-codex` (ChatGPT) Responses requests, matching codex-rs per-turn `cyberAccessProgram`: `standard`, `daybreak_blue`, or `daybreak_red`. `auto` omits the field so the backend picks a compatible program for the model and your access. Every agent conversation in the session sends it: main, advisor, and subagent turns, their side and handoff requests, and their compaction (V2 remote and local summaries). One-shot helpers outside those conversations (session titles, commit messages, memory extraction, edit auto-repair) never do. When the model's Codex catalog entry lists the programs it accepts, any other program is dropped instead of sent. Other providers, legacy `/responses/compact` requests, and the pi-native gateway never carry it. The backend decides whether your workspace is authorized for the program. |
