...
| `providers.openai-codex.cyberAccess`         | enum    | `auto`            | Cyber access program sent as `access_programs.cyber` on first-party `openai-codex` (ChatGPT) Responses requests: session, advisor, and subagent turns, side requests, and compaction (V2 remote and local summaries), matching codex-rs per-turn `cyberAccessProgram`. `auto` omits the field so the backend applies its default treatment; `standard`, `daybreak-blue`, and `daybreak-red` request that program. The backend decides whether your workspace is authorized for it. Other providers and legacy `/responses/compact` requests never carry it, and the pi-native gateway drops it. |
