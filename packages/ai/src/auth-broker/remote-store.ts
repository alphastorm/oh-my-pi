...
export class RemoteAuthCredentialStore implements AuthCredentialStore {
	...
	#withoutLocallyDisabled<T extends { credentials: readonly SnapshotEntry[] }>(snapshot: T): T {
		...
		return { ...snapshot, credentials: snapshot.credentials.filter(entry => !this.#locallyDisabledAt.has(entry.id)) };
	}
	...
	async deleteAuthCredentialsRemote(provider: string, disabledCause: string): Promise<void> {
		...
		// Same two guards the single-row disable applies: record the rows so an
		// `entry` frame already queued on the stream's separate connection cannot
		// re-add a logged-out credential, and persist the filtered snapshot when
		// an active stream suppresses the refresh below.
		for (const entry of this.#snapshot.credentials) {
			if (entry.provider === provider) this.#locallyDisabledAt.set(entry.id, this.#generation);
		}
		this.#removeProviderEntries(provider);
		if (this.#streamingActive) this.#persistCurrentSnapshot();
