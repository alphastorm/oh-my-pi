...
const PROFILE_IDS = [
	...
	"linux-docker-local",
] as const satisfies readonly ApplianceProfileId[];
const ADAPTERS = [
	"darwin-remote-ssh",
	"windows-docker-local",
	"linux-docker-local",
] as const satisfies readonly ApplianceAdapterId[];
const STATUSES = [
	"qualified",
	"preview",
	"blocked",
	"unsupported",
] as const satisfies readonly ApplianceSupportStatus[];
const ACTIONS = [
	"doctor",
	"plan",
	"install",
...
function parseProfile(value: unknown): ApplianceProfile {
	...
	if (item.silent_cloud_fallback !== false)
		throw new Error("Compatibility profile must disable silent cloud fallback");
	if ((adapter === "darwin-remote-ssh") !== (transport === "ssh-loopback"))
		throw new Error("Profile transport does not match its adapter");
	...
	if (commands.length === 0 || new Set(commands).size !== commands.length)
		throw new Error("Profile lifecycle commands are absent or duplicated");
	...
	if (
		!OCI_DIGEST.test(imageDigest) ||
		!imageReference.includes(`@${imageDigest}`) ||
		/:latest(?:@|$)/u.test(imageReference)
	) {
	...
	for (const digest of [modelSha256, configurationSha256, serverBinarySha256])
		if (!SHA256.test(digest)) throw new Error("Runtime identity contains an invalid SHA-256");
	...
	const acceptance =
		item.acceptance_receipt == null ? undefined : receipt(item.acceptance_receipt, "Acceptance receipt");
	const gpuReceipt = receipt(gpu.receipt, "GPU qualification receipt");
	const installable = item.installable === true;
	if (status === "qualified" && (!acceptance || gpuStatus !== "qualified" || !installable))
		throw new Error("Qualified profile lacks complete acceptance or GPU qualification evidence");
	if ((status === "blocked" || status === "unsupported") && installable)
		throw new Error("Blocked profile cannot be installable");
	...
	if (capabilities.some(capability => !knownCapabilities.includes(capability)))
		throw new Error("Runtime contains an unknown capability");
	...
	if (contextWindow !== 131072 || maxTokens !== 32768 || concurrency !== 1)
		throw new Error("Runtime identity declares unsupported context, output, or concurrency bounds");
	const architecture = oneOf(runtime.cuda_architecture, ["sm_120a", "sm_89"] as const, "CUDA architecture");
	if (runtime.vision !== capabilities.includes("vision") || typeof runtime.preserve_thinking !== "boolean")
		throw new Error("Runtime behavior flags disagree with capabilities");
	return {
		id: "qwen3.8-27b",
		profile,
		runtime: "ninfer",
		architecture,
		minVramGiB: integer(runtime.minimum_vram_gib, "Minimum VRAM"),
		minimumDiskGiB: integer(runtime.minimum_disk_gib, "Minimum disk"),
		...
		concurrency,
		preserveThinking: runtime.preserve_thinking,
		protocol: "openai-responses",
		capabilities: capabilities as ApplianceProfile["capabilities"],
		release: text(item.product_release, "Product release"),
		servedModel: "q38-ninfer",
		aliases: strings(item.aliases, "Profile aliases"),
		defaultPort: integer(item.local_port, "Local port"),
		availability: {
			installable,
			channel: status === "qualified" ? "released" : "beta",
		...
		assets: {
			...
			model: {
				...
				url: modelUrl,
				sha256: modelSha256,
				bytes: integer(runtime.model_bytes, "Model bytes"),
			},
		},
		adapter,
		supportStatus: status,
		supportOwner: text(item.support_owner, "Support owner"),
		limitations: strings(item.limitations, "Profile limitations"),
		...
		container: {
			imageReference,
			imageDigest,
			containerPort: integer(item.container_port, "Container port"),
			serverBinarySha256,
			configurationSha256,
			restartPolicy: "no",
		},
...
export async function loadCompatibilityAuthority(
	path: string,
	expectedSha256: string,
): Promise<CompatibilityAuthority> {
	...
	if (new Set(profiles.map(profile => profile.profile)).size !== profiles.length)
		throw new Error("Compatibility authority contains duplicate profiles");
