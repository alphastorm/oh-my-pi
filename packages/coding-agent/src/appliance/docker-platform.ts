...
import { BoundedApplianceExecutor, type BoundedCommandRunner } from "./bounded-executor";
import type {
	ApplianceAsset,
...
import { inspectNativeWindowsDocker, validateWindowsLocalPath, windowsSystem32Path } from "./windows-docker-detection";
...
function record(value: unknown): Record<string, unknown> {
	if (!value || typeof value !== "object" || Array.isArray(value))
		throw new Error("NInfer lifecycle returned a non-object receipt");
...
function parseLifecycle(stdout: string, operation: string): LifecycleResult {
	...
	try {
		...
		throw new Error("NInfer lifecycle returned invalid JSON");
	}
	const receipt = record(value);
	if (
		receipt.schemaVersion !== 1 ||
		receipt.kind !== "omp-ninfer-lifecycle-receipt" ||
	...
	return {
		...
		...(receipt.configurationSha256 !== undefined
			? { configurationSha256: safe(receipt.configurationSha256, "Configuration digest") }
			: {}),
		...(receipt.serverBinarySha256 !== undefined
			? { serverBinarySha256: safe(receipt.serverBinarySha256, "Server digest") }
			: {}),
...
export function buildWindowsLifecycleCommand(script: string, args: string[], systemRoot?: string): string[] {
	...
	return [
		...
		"Bypass",
		"-File",
		validateWindowsLocalPath(script, "Lifecycle script path"),
		...args,
	];
}
...
export function detectNativeLinuxDocker(input: {
	...
	const exactWslDelegation =
		input.wslInterop !== undefined &&
		input.delegatedWslDistribution !== undefined &&
...
abstract class DockerLocalPlatform implements AppliancePlatform {
	...
	protected constructor(options: DockerPlatformOptions) {
		if (!options.profile.container || !options.profile.lifecycle || !options.profile.assets)
			throw new Error("Docker profile lacks lifecycle authority");
	...
	protected abstract platformFacts(): Promise<{
		os: NodeJS.Platform;
		architecture: string;
		dockerPath: string;
		blockers: string[];
	...
	protected async hardenRoot(): Promise<void> {
	...
	async #scriptPath(acquire = true): Promise<string | undefined> {
		...
		const target = path.join(
			this.root,
			`lifecycle-${lifecycle.scriptSha256}${this.profile.adapter === "windows-docker-local" ? ".ps1" : ".sh"}`,
		);
		try {
			if (
				createHash("sha256")
					.update(await readFile(target))
					.digest("hex") === lifecycle.scriptSha256
			)
				return target;
		} catch {}
		if (!acquire) return undefined;
		await this.hardenRoot();
		const response = await this.#fetch(lifecycle.scriptUrl, {
			redirect: "follow",
			signal: AbortSignal.timeout(120_000),
		});
		if (!response.ok || new URL(response.url).protocol !== "https:")
			throw new Error("NInfer lifecycle download failed");
		const bytes = new Uint8Array(await response.arrayBuffer());
		if (createHash("sha256").update(bytes).digest("hex") !== lifecycle.scriptSha256)
			throw new Error("NInfer lifecycle checksum mismatch");
		try {
	...
	async #invoke(operation: string, args: string[] = [], acquireScript = true): Promise<LifecycleResult> {
		...
		const command = this.lifecycleCommand(script, [
			...this.profile.lifecycle!.arguments,
			operation,
			"--profile",
		...
		if (result.code !== 0 && receipt.status === "ok")
			throw new Error("NInfer lifecycle exited nonzero with ok status");
	...
	async inspectHost(): Promise<ApplianceHostFacts> {
		...
		if (facts.blockers.length === 0) {
			const image = await this.executor.run([
				docker,
				"image",
				"inspect",
				this.profile.container!.imageReference,
				"--format",
				"{{json .RepoDigests}}",
			]);
			const imagePresent = image.code === 0 && image.stdout.includes(this.profile.container!.imageDigest);
			if (imagePresent) {
				const gpu = await this.executor.run([
					docker,
					"run",
					"--rm",
					"--pull=never",
				...
				if (gpu.code === 0) {
					...
					gpus = gpu.stdout
						...
						.flatMap(line => {
							...
							return model && Number.isFinite(mib)
								? [{ model, vramGiB: Math.round((mib / 1024) * 10) / 10, computeCapability }]
								: [];
						});
				}
			}
		...
		try {
			const value = await statfs(path.dirname(this.root));
			freeDiskGiB = gib(value.bavail * value.bsize);
		} catch {}
		return {
			os: facts.os,
	...
	async artifactPresent(asset: ApplianceAsset): Promise<boolean> {
		...
		try {
			const result = await this.#invoke("artifact-status", ["--kind", "model", "--sha256", asset.sha256], false);
	...
	async removeArtifact(_ref: string): Promise<void> {
		/* immutable shared assets are never removed by OMP */
	}
	...
	async startCandidate(candidate: ApplianceCandidate): Promise<void> {
		await this.#invoke("start", ["--handle", candidate.handle]);
	}
	async stopCandidate(candidateHandle: string): Promise<void> {
		await this.#invoke("stop", ["--handle", candidateHandle]);
	}
	...
	async startInstallation(installation: ApplianceInstallation, _secret: string): Promise<void> {
		await this.#invoke("restore", ["--handle", installation.candidateHandle]);
	}
	async #status(target: ApplianceCandidate | ApplianceInstallation, secret: string): Promise<Record<string, unknown>> {
		...
		const expectedBinding = dockerLoopbackBinding(
			"port" in target ? target.port : target.route.port,
			this.profile.container!.containerPort,
		);
		if (
			!lifecycle.containerId ||
			lifecycle.imageDigest !== this.profile.container!.imageDigest ||
	...
	async probeHealth(target: ApplianceCandidate | ApplianceInstallation, secret: string): Promise<void> {
		...
		if (
			status.image_digest !== this.profile.container!.imageDigest ||
			status.model_sha256 !== this.profile.assets!.model.sha256 ||
			status.configuration_sha256 !== this.profile.container!.configurationSha256 ||
			status.server_binary_sha256 !== this.profile.container!.serverBinarySha256
		) {
	...
	async probeProtocol(target: ApplianceCandidate | ApplianceInstallation, secret: string): Promise<void> {
		await this.probeHealth(target, secret);
	}
	async quickQualification(
		...
		await this.probeHealth(target, secret);
		const item: ApplianceBenchmarkCase = {
			name: "short-decode",
			ok: true,
			durationMs: Math.round((performance.now() - started) * 10) / 10,
			oracleSha256: createHash("sha256").update("authenticated-local-status").digest("hex"),
		};
		return { ok: true, cases: [item] };
	}
	async probeRoutedRequest(installation: ApplianceInstallation, secret: string): Promise<void> {
		await this.probeHealth(installation, secret);
	}
	async readEndpointStatus(installation: ApplianceInstallation, secret: string): Promise<ApplianceEndpointStatus> {
		...
		return {
			fingerprint: createHash("sha256")
				.update(
					installation.route.baseUrl +
						"\0" +
						this.profile.container!.imageDigest +
						"\0" +
						this.profile.assets!.model.sha256 +
						"\0" +
						this.profile.container!.configurationSha256,
				)
				.digest("hex"),
	...
	async checkpoint(): Promise<never> {
		throw new Error("Docker local compatibility profile does not declare durable checkpoint ownership");
	}
}

export class WindowsDockerAppliancePlatform extends DockerLocalPlatform {
	// biome-ignore lint/complexity/noUselessConstructor: This constructor intentionally widens the protected base constructor to public.
	constructor(options: DockerPlatformOptions) {
		super(options);
	}
	...
	override async inspectHost(): Promise<ApplianceHostFacts> {
		...
		try {
			const result = await this.executor.run([
				...
				"-NonInteractive",
				"-Command",
				`[Console]::Out.Write((Get-PSDrive -Name ${drive}).Free)`,
			]);
	...
	override async isPortOccupied(port: number): Promise<boolean> {
		...
		const powershell = windowsPowerShellPath(process.env.SystemRoot);
		try {
			const listener = await this.executor.run([
				powershell,
				"-NoLogo",
				"-NoProfile",
				"-NonInteractive",
				"-Command",
				`[Console]::Out.Write([bool](Get-NetTCPConnection -State Listen -LocalPort ${port} -ErrorAction SilentlyContinue))`,
			]);
			if (listener.code !== 0 || parseWindowsPortOccupied(listener.stdout)) return true;
			const containers = await this.executor.run([
				facts.dockerPath,
				"ps",
				"--filter",
				`publish=${port}`,
	...
	protected async platformFacts(): Promise<{
		os: NodeJS.Platform;
		architecture: string;
		dockerPath: string;
		blockers: string[];
		...
		return {
			...
			dockerPath: detection.dockerPath ?? "",
