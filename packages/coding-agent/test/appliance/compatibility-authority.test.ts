...
function profile(
	id: "darwin-remote-ssh" | "windows-docker-local" | "linux-docker-local",
	status: "qualified" | "preview" = "preview",
) {
	return {
		id,
		adapter: id,
		...
		installable: status === "qualified" || id !== "darwin-remote-ssh",
		support_owner: "omp-ninfer",
		product_release: "v0.2.0-preview",
		aliases: ["local-max"],
		local_port: 18089,
		container_port: 8080,
		limitations: status === "qualified" ? [] : ["Preview pending real client acceptance"],
		blockers: [],
		...
		runtime: {
			image_reference: `ghcr.io/alphastorm/ninfer@sha256:${"c".repeat(64)}`,
			image_digest: `sha256:${"c".repeat(64)}`,
			model_url: "https://example.invalid/model.ninfer",
			model_bytes: 18_210_531_328,
		...
		lifecycle: {
			script_url: `https://example.invalid/lifecycle${id === "windows-docker-local" ? ".ps1" : ".sh"}`,
...
async function authority(profiles: unknown[]): Promise<{ path: string; sha256: string }> {
	const root = await mkdtemp(join(tmpdir(), "omp-compatibility-"));
	roots.push(root);
...
describe("omp-ninfer compatibility authority", () => {
	test("binds the closed adapter set without an OS GPU matrix", async () => {
		const fixture = await authority([
			profile("darwin-remote-ssh", "qualified"),
		...
		expect(loaded.profiles.map(item => item.adapter)).toEqual([
			"darwin-remote-ssh",
	...
	test("rejects drift and every unknown authority discriminator", async () => {
		...
		for (const mutation of [
			{ schema_version: 2, authority_id: "x", profiles: [] },
			{
				schema_version: 1,
				authority_id: "x",
				profiles: [{ ...profile("windows-docker-local"), adapter: "generic" }],
			},
			{
				schema_version: 1,
				authority_id: "x",
				profiles: [{ ...profile("windows-docker-local"), transport: "cloud" }],
			},
			{
				schema_version: 1,
				authority_id: "x",
				profiles: [{ ...profile("windows-docker-local"), commands: ["repair"] }],
			},
		]) {
			const next = await authority((mutation as { profiles: unknown[] }).profiles);
			const bytes = Buffer.from(JSON.stringify(mutation));
			await writeFile(next.path, bytes);
	...
	test("rejects mutable images and incomplete qualified claims", async () => {
		const mutable = profile("windows-docker-local");
		mutable.runtime.image_reference = "ghcr.io/alphastorm/ninfer:latest";
		...
		const incomplete = profile("windows-docker-local", "qualified");
		incomplete.acceptance_receipt = null;
