...
} from "@oh-my-pi/pi-coding-agent/appliance/bounded-executor";
import {
...
function linuxDockerProfile(): ApplianceProfile {
	return {
		...
		availability: {
			...
			channel: "beta",
			blockers: [],
			qualificationReceipt: { url: "https://example.invalid/acceptance.json", sha256: "3".repeat(64) },
		},
		assets: {
			runtime: {
				kind: "runtime",
				url: `ghcr.io/alphastorm/ninfer@sha256:${"4".repeat(64)}`,
				sha256: "4".repeat(64),
			},
			model: {
				kind: "model",
				url: "https://example.invalid/model.ninfer",
				sha256: "2".repeat(64),
				bytes: 18_210_531_328,
			},
		},
		adapter: "linux-docker-local",
		supportStatus: "preview",
		supportOwner: "omp-ninfer",
		limitations: [],
		lifecycleCommands: ["doctor", "plan", "install", "status", "benchmark", "rollback", "support-bundle"],
		container: {
			imageReference: `ghcr.io/alphastorm/ninfer@sha256:${"4".repeat(64)}`,
			imageDigest: `sha256:${"4".repeat(64)}`,
			containerPort: 8080,
			serverBinarySha256: "5".repeat(64),
			configurationSha256: "6".repeat(64),
			restartPolicy: "no",
		},
		lifecycle: {
			scriptUrl: "https://example.invalid/lifecycle.sh",
			scriptSha256: lifecycleScriptSha256,
			arguments: [],
		},
	};
}
...
describe("explicit local Docker adapters", () => {
	test("builds exact native Windows PowerShell arguments", () => {
		const command = buildWindowsLifecycleCommand(
			"C:\\Users\\tester\\lifecycle.ps1",
			["install", "--json"],
			"C:\\Windows",
		);
		...
		expect(() => buildWindowsLifecycleCommand("C:\\Users\\tester\\lifecycle.ps1", [], "C:Windows")).toThrow(
			"local absolute",
		);
	...
	test("hardens Windows ACLs before adapter secret writes", async () => {
		...
		const runner = async (command: readonly string[]): Promise<BoundedCommandResult> => {
			...
			if (command[0] === "C:\\Windows\\System32\\whoami.exe")
				return { code: 0, stdout: '"tester","S-1-5-21-1-2-3-1001"\r\n', stderr: "" };
		...
		await expect(
			hardenWindowsApplianceRoot(root, new BoundedApplianceExecutor({ runner }), "C:Windows"),
	...
	test("detects native Linux without coupling to GPU model", () => {
		...
		).toContain("linux-docker-local requires native Linux OMP");
		expect(
			detectNativeLinuxDocker({
				platform: "linux",
				architecture: "x64",
		...
		).toContain("NVIDIA Container Toolkit is unavailable");
		expect(() =>
			withHostPlatform(
				"darwin",
				undefined,
				() =>
					new LinuxDockerAppliancePlatform({
						agentDir: ".",
	...
	test("delegates immutable acquisition and candidate setup without exposing secrets", async () => {
		...
		const runner = async (command: readonly string[]): Promise<BoundedCommandResult> => {
			...
			const operation =
				command.find(value => ["acquire", "prepare", "start", "status"].includes(value)) ?? "unknown";
			...
			return {
				...
				stdout: JSON.stringify({
					schemaVersion: 1,
					kind: "omp-ninfer-lifecycle-receipt",
					operation,
					status: "ok",
					...
					...(operation === "status"
						? {
								...
								imageDigest: profile.container!.imageDigest,
								modelSha256: profile.assets!.model.sha256,
								configurationSha256: profile.container!.configurationSha256,
								serverBinarySha256: profile.container!.serverBinarySha256,
								binding: "127.0.0.1:18089:8080",
								restartPolicy: "no",
								owned: true,
							}
						: {}),
			...
			String(input).endsWith("/status")
				? {
						...
						json: async () => ({
							...
							image_digest: profile.container!.imageDigest,
							model_sha256: profile.assets!.model.sha256,
							configuration_sha256: profile.container!.configurationSha256,
							server_binary_sha256: profile.container!.serverBinarySha256,
						}),
					}
				: {
						ok: true,
						...
						arrayBuffer: async () =>
							lifecycleScript.buffer.slice(
								lifecycleScript.byteOffset,
								lifecycleScript.byteOffset + lifecycleScript.byteLength,
							),
					}) as unknown as typeof fetch;
		const platform = withHostPlatform(
			"linux",
			undefined,
			() => new LinuxDockerAppliancePlatform({ agentDir: root, root, profile, runner, fetch: fakeFetch }),
		);
	...
	test("rejects wrong existing lifecycle-script bytes after exclusive-create collision", async () => {
			...
			() =>
				new LinuxDockerAppliancePlatform({
					...
					runner: async command => {
						commands.push([...command]);
						throw new Error("lifecycle must not execute");
					},
					fetch: (async () => ({
						ok: true,
						url: "https://example.invalid/lifecycle.sh",
		...
		await expect(platform.acquireArtifact(profile.assets!.runtime)).rejects.toThrow(
			"checksum mismatch for existing script",
		);
		expect(commands).toEqual([]);
	});
});
