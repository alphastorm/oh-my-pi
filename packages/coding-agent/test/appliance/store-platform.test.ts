...
describe("platform-aware appliance store", () => {
	test("hardens the product root before the first secret write", async () => {
		const parent = await mkdtemp(join(tmpdir(), "omp-store-platform-")); roots.push(parent);
		...
		await store.writeReceipt({ schemaVersion: 1, receiptId: "receipt-1", action: "install", status: "ok", timestamp: new Date(0).toISOString(), details: {} });
	...
	test("round-trips a durable interrupted transition", async () => {
		const parent = await mkdtemp(join(tmpdir(), "omp-store-pending-")); roots.push(parent);
		const store = new FileApplianceStore(parent);
		await store.writeState({
			schemaVersion: 1, revision: 1,
			pending: { action: "install", stage: "before-predecessor-stop", installationId: "candidate-1", profile: "windows-docker-local", failureReceiptId: "failure-1" },
		}, 0);
		expect(await store.readState()).toMatchObject({ revision: 1, pending: { action: "install", stage: "before-predecessor-stop", profile: "windows-docker-local", failureReceiptId: "failure-1" } });
	});
});
