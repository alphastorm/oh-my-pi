...
describe("runVisionRequestOnce", () => {
	it("shares successful work only inside one logical turn", async () => {
		...
		const first = runVisionRequestOnce(sessionId, "image_question", "turn-1", "key", request);
		const duplicate = runVisionRequestOnce(sessionId, "image_question", "turn-1", "key", request, () => {
			reuses += 1;
		});
		expect(calls).toBe(1);
		resolve("answer");
		...
		const later = runVisionRequestOnce(sessionId, "image_question", "turn-2", "key", async () => {
			calls += 1;
			return "later";
		});
	...
	it("keeps attachment and inspect-image scope changes isolated", async () => {
		...
		const inspect = runVisionRequestOnce(sessionId, "image_question", "inspect-scope", "key", async () => "inspect");
	...
	it("evicts failed requests so a later attempt can retry", async () => {
		...
		let calls = 0;
		await expect(
			runVisionRequestOnce(sessionId, "image_question", "turn-1", "key", async () => {
				calls += 1;
				throw new Error("provider failed");
			}),
		).rejects.toThrow("provider failed");

		const retried = await runVisionRequestOnce(sessionId, "image_question", "turn-1", "key", async () => {
			calls += 1;
			return "recovered";
		});
